...
if [[ -n "$SOURCE_ROOT" && -f "$SOURCE_ROOT/src/cli/index.ts" ]]; then
  if command -v bun >/dev/null 2>&1; then
    exec bun "$SOURCE_ROOT/src/cli/index.ts" run harness-trace-grade "$@"
  fi
fi

if command -v repo-harness >/dev/null 2>&1; then
  exec repo-harness run harness-trace-grade "$@"
fi

echo "Missing repo-harness CLI for helper harness-trace-grade" >&2
exit 1
